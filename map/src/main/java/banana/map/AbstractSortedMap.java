...
public abstract class AbstractSortedMap<K,V> extends AbstractMap<K,V> implements SortedMap<K,V>
{
    ...
    /**
     * Utility for determining if a given key is invalid
     * We check if the key can be compared to itself, if it can its valid
     * If it can't be compared to itself then it is invalid in which case we throw an Exception
     * @param key key
     * @return boolean
    ...
    protected boolean checkKey(K key) throws IllegalArgumentException {
        try {
            ...
            throw new IllegalArgumentException("Given key is incompatible and cannot be used for comparisons using the current comparator");
        }
    }
}
